...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    // 1. Create and start listener
    val listener = createListener(relativePath)
    val listenerSubscription = listener.start()

    // 2. Create cancellable computation using Future.run
    val serverSubscription = Future.run() { ct =>
      // 3. While not cancelled, await next request and respond asynchronously
      async {
    ...
    // Return a subscription that cancels both listener and server loop
